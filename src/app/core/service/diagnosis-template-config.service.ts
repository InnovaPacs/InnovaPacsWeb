...
export class DiagnosisTemplateConfigService {
  ...
  public findTemplate() {
    return this.http.get(`${this.API}api/v1/diagnosisPdfConfiguration/user`, {'headers': this.authService.getHeader()});
  }

  public save(template: DiagnosisTemplateConfig) {
    return this.http.post(`${this.API}api/v1/diagnosisPdfConfiguration`, 
    template,
    {'headers': this.authService.getHeader()});
  }

  public update(template: DiagnosisTemplateConfig) {
    return this.http.put(`${this.API}api/v1/diagnosisPdfConfiguration`, 
    template,
    {'headers': this.authService.getHeader()});
  }
}
